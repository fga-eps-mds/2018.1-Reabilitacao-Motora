...
namespace pontosmovimentofisioterapeuta
{
    public class PontosMovimentoFisioterapeuta
    {
        ...
        public PontosMovimentoFisioterapeuta (string caminho)
        {
            path = caminho;
            using (banco.conn = new SqliteConnection(path))
            {
                ...
                banco.sqlQuery = "CREATE TABLE IF NOT EXISTS PONTOSMOVIMENTOFISIOTERAPEUTA (idMovimentoFisioterapeuta INTEGER primary key AUTOINCREMENT,idMovimento INTEGER not null,tempo REAL not null,anguloDeJunta REAL not null,foreign key (idMovimento) references MOVIMENTO (idMovimento));";
            ...
            double anguloDeJunta)
        {
            using (banco.conn = new SqliteConnection(path))
            {
                banco.conn.Open();
                banco.cmd = banco.conn.CreateCommand();
                banco.sqlQuery = "insert into PONTOSMOVIMENTOFISIOTERAPEUTA (";
        ...
        public void Read ()
        {
            using (banco.conn = new SqliteConnection(path))
            {
                banco.conn.Open();
                banco.cmd = banco.conn.CreateCommand();
                banco.sqlQuery = "SELECT * " + "FROM PONTOSMOVIMENTOFISIOTERAPEUTA";
                banco.cmd.CommandText = banco.sqlQuery;
                ...
                while (reader.Read())
                {
                    ...
                    Debug.Log (string.Format("\"{0}\" = ", tt.TABLES[tableId].colName[0]) + idMovimentoFisioterapeuta +
