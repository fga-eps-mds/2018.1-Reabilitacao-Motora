...
/Applications/Unity/Unity.app/Contents/MacOS/Unity -batchmode -projectPath $(pwd) -logFile /dev/stdout -buildOSXUniversalPlayer "Build/osx/$project.app" -quit
echo "${TEXTOVERDE} ======================================= BUILD CONCLUÍDA PARA OSX ======================================= ${NORMAL}"

echo "${TEXTOAMARELO} Tentativa de Build do $project para Linux  ${NORMAL}"
/Applications/Unity/Unity.app/Contents/MacOS/Unity -batchmode -projectPath $(pwd) -logFile /dev/stdout -buildLinuxUniversalPlayer "Build/linux/$project.exe" -quit
echo "${TEXTOVERDE} ======================================= BUILD CONCLUÍDA PARA LINUX ======================================= ${NORMAL}"

echo "${TEXTOAMARELO} Tentativa de Build do $project para Windows ${NORMAL}"
/Applications/Unity/Unity.app/Contents/MacOS/Unity -batchmode -projectPath $(pwd) -logFile /dev/stdout -buildWindowsPlayer "Build/windows/$project.exe" -quit
