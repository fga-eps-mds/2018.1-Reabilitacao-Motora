        ...
        * Função que atualiza dados já cadastrados anteriormente na relação de pontosrotulopaciente.
            ...
            double tempoFinal)
        {
            ...
            using (banco.conn = new SqliteConnection(GlobalController.instance.path))
            {
                ...
                banco.sqlQuery += string.Format("\"{0}\"=\"{1}\",", TablesManager.instance.Tables[tableId].colName[2], estagioMovimentoPaciente);
                banco.sqlQuery += string.Format("\"{0}\"=\"{1}\",", TablesManager.instance.Tables[tableId].colName[3], tempoInicial);
                banco.sqlQuery += string.Format("\"{0}\"=\"{1}\" ", TablesManager.instance.Tables[tableId].colName[4], tempoFinal);
