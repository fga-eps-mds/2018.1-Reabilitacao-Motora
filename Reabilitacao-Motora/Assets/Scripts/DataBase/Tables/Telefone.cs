    ...
    public class Telefone
    {
        ...
        public void Update(int idPessoa, string telefone)
        {
            using (banco.conn = new SqliteConnection(GlobalController.instance.path))
            {
                ...
                banco.sqlQuery += string.Format("\"{0}\"=\"{1}\",", TablesManager.instance.Tables[tableId].colName[0], idPessoa);
                banco.sqlQuery += string.Format("\"{0}\"=\"{1}\" ", TablesManager.instance.Tables[tableId].colName[1], telefone);

                banco.sqlQuery += string.Format("WHERE \"{0}\"=\"{1}\" AND \"{2}\"=\"{3}\"", TablesManager.instance.Tables[tableId].colName[0], idPessoa, TablesManager.instance.Tables[tableId].colName[1], telefone);
